...
void solve(int TEST_CASE){
    ...
    string s, t;
    cin >> k >> x;
    int Total = (k*k + k)/2 + (k*k - k)/2;
    if(x>=Total)Print_Return(k*2-1);
    int l=0, r=2*k-2; // possible answers 
    function<int(int)> F = [&](int x){
        return (x*(x+1))/2;
    };
    while(l<=r){
        int mid = (l + r)/2;
        // basically thinking of mid as the answer so two cases mid>=k or mid<k
        int sum = F(mid);
        if(mid>=k){
            sum = F(k); // till k;
            // rem sum : [k-1 to Need ] // (mid + Need = 2*k)
            sum += F(k-1) - F(2*k-1-mid);
        }
        // w(l, r, mid, k, ians, sum);
        if(sum<x){
            ians = max(ians, mid);
            l = mid + 1;
        }
