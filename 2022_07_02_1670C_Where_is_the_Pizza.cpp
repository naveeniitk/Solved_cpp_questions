...
int p(int a, int b=mod-2, int m=mod){
    int ans = 1;
    while(b>0){
        if(b&1)ans = (ans*a)%m;
        a = (a*a)%m;
...
const int mxn = 1e5 + 5;
vc<int> Parent(mxn, 0), Size(mxn, 0);
...
void solve(int TEST_CASE){
    ...
    function<int(int)> Find_Parent = [&](int i){
        if(Parent[Parent[i]]!=Parent[i])Parent[i] = Find_Parent(Parent[i]);
        return Parent[i];
    };

    function<void(int, int)> Union_Nodes = [&](int a, int b){
        int ap = Find_Parent(a), bp = Find_Parent(b);
        if(ap!=bp){
    ...
    Fo(i, 0, n-1){
        s.insert(Find_Parent(a[i]));
    }

    Fo(i, 0, n-1){
        int x; cin >> x;
        if(x || a[i]==b[i]){
            if(sz(s)>0)s.erase(Find_Parent(a[i]));
        }
    }
