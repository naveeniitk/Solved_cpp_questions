...
void solve(int TEST_CASE){
    ...
    vc<ar<int,2>> v(n+1, {0, 0});
    
    Fo(i, 1, n){
        cin >> v[i][0] >> v[i][1];
    }

    // for a node, all of its children will be part of some query 1 to that child(c1, c2, c3..) say( 1 to c1 and 1 to c2 and 1 to c3)
    // now the parent of (c1, c2, c3) i.e. current node must be added the values as Parent is also part of the sequence of updating c1, c2, c3.
    // if all the updates done in (c1, c2, c3..) are unable to make value of Parent in its range (< Min value of node) then one extra operation is req
    // then just make this node's value to be maximum permissible as it will be the last in the path of its update
    ...
    function<void(int, int)> dfs = [&](int u, int p){
        ...
        int uLmin = v[u][0], uRmax = v[u][1];
        if(Maxx[u] < uLmin){
            ians++;
            Maxx[u] = uRmax;
            return;
        }
        Maxx[u] = min(Maxx[u], uRmax);
    };
