...
void solve(int TEST_CASE){
    ...
    map<ar<int,2>, int> g;
    map<int, ar<int,2>> Layer;
    function<int(int, int)> G = [&](int i, int j){
        return a[i][j];
    };
    function<int(int, int)> F = [&](int i, int j){
        ...
        g[{i, j}] += 1;
        if(g[{j, n-1-i}]==0)ans += G(j, n-1-i), cnt++;
        g[{j, n-1-i}] += 1;
        if(g[{n-1-i, n-1-j}]==0)ans += G(n-1-i, n-1-j), cnt++;
        g[{n-1-i, n-j-1}] += 1;
        if(g[{n-j-1, i}]==0)ans += G(n-j-1, i), cnt++;
        g[{n-1-j, i}] += 1;
