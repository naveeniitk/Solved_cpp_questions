...
void solve(int TEST_CASE){
    ...
    if(sumT>maxT || sumT<minT)Print_Return("NO");
    cout << "YES\n";
    int Cur = minT;
    // just increase the T[i][0] to a greater value if Cur haven't reached sumT
    Fo(i, 0, d-1){
        while(T[i][0] < T[i][1]){
            if(Cur==sumT)break;
            T[i][0] += 1;
