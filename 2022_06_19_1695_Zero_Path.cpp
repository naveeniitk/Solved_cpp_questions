...
void solve(int TEST_CASE){
    ...
    f[{0, 0}] = {a[0][0], a[0][0]};
    Fo(i, 1, m-1){
        f[{0, i}] = {f[{0, i-1}][0] + a[0][i], f[{0, i-1}][1] + a[0][i]};
    }
    Fo(j, 1, n-1){
        f[{j, 0}] = {f[{j-1, 0}][0] + a[j][0], f[{j-1, 0}][1] + a[j][0]};
    }
    Fo(i, 1, n-1){
        Fo(j, 1, m-1){
            ar<int,2> Left = f[{i-1, j}], Rite = f[{i, j-1}];
            x = min(Left[0], Rite[0]), y = max(Left[1], Rite[1]);
            f[{i, j}] = {x + a[i][j], y + a[i][j]};
        }
    }
    // w(f[{n-1, m-1}]);
    ar<int,2> ans = f[{n-1, m-1}];
    if(((ans[0]&1)^1) && ans[0]<=0 && ((ans[1]&1)^1) && ans[1]>=0)Print_Return("YES")
    else cout << "NO\n";
}    
